...
import { Instagram, Globe, Mail, MapPin, ArrowRight, Facebook, Twitter } from "lucide-react";
...
export default function ArtisanDetailPage({ params }) {
  ...
  useEffect(() => {
    const fetchArtisan = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/api/user/getArtisan/${params.id}`);
  ...
  const artisanName = artisan ? `${artisan.firstname} ${artisan.lastname}` : "Unknown Artisan";
  ...
  const artisanGallery = artisan?.gallery?.length > 0 ? artisan.gallery : ["/placeholder.svg"];
  
  // Default process steps if none available
  const process = artisan?.process || [
    {
      title: "Design",
      description: "The artisan designs each piece with care and attention to detail.",
    },
    {
      title: "Creation",
    ...
    {
      title: "Quality Check",
      description: "Every item undergoes careful inspection to ensure top quality.",
    },
    {
      title: "Finishing",
    ...
    <div className="pt-24 bg-dark-900 min-h-screen">
      ...
      <section className="relative h-[60vh] flex items-center">
        <div className="absolute inset-0 z-0">
          <Image src={artisanImage} alt={artisanName} fill className="object-cover" />
        ...
        <div className="container mx-auto px-8 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-5xl font-display text-white mt-2 mb-4">{artisanName}</h1>
            ...
            <div className="flex space-x-4 mt-8">
              {artisan?.socialMedia?.instagram && (
                <a href={artisan.socialMedia.instagram} target="_blank" rel="noopener noreferrer" className="bg-dark-800/80 backdrop-blur-sm text-white p-2 rounded-full hover:bg-accent-green hover:text-dark-900 transition-colors">
                  <Instagram className="w-5 h-5" />
                </a>
              )}
              {artisan?.socialMedia?.website && (
                <a href={artisan.socialMedia.website} target="_blank" rel="noopener noreferrer" className="bg-dark-800/80 backdrop-blur-sm text-white p-2 rounded-full hover:bg-accent-green hover:text-dark-900 transition-colors">
                  <Globe className="w-5 h-5" />
                </a>
              )}
              {artisan?.socialMedia?.facebook && (
                <a href={artisan.socialMedia.facebook} target="_blank" rel="noopener noreferrer" className="bg-dark-800/80 backdrop-blur-sm text-white p-2 rounded-full hover:bg-accent-green hover:text-dark-900 transition-colors">
                  <Facebook className="w-5 h-5" />
                </a>
              )}
              {artisan?.socialMedia?.twitter && (
                <a href={artisan.socialMedia.twitter} target="_blank" rel="noopener noreferrer" className="bg-dark-800/80 backdrop-blur-sm text-white p-2 rounded-full hover:bg-accent-green hover:text-dark-900 transition-colors">
                  <Twitter className="w-5 h-5" />
                </a>
              )}
              <a href={`mailto:${artisan?.email}`} className="bg-dark-800/80 backdrop-blur-sm text-white p-2 rounded-full hover:bg-accent-green hover:text-dark-900 transition-colors">
      ...
      <section className="py-16 bg-dark-900">
        <div className="container mx-auto px-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-16">
                <div>
                  ...
                  <p className="text-white/70">
                    {artisan?.additionalBio || "This artisan creates pieces that showcase their unique talent and creativity."}
                  </p>
                </div>
          ...
          {activeTab === "products" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
              <h2 className="text-3xl font-display text-white mb-8">Products by {artisanName}</h2>
              {products.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {products.map((product, index) => (
                    <div key={product._id || index} className="bg-dark-800 rounded-lg overflow-hidden group">
                      ...
                      <div className="p-6">
                        <h3 className="text-lg font-medium text-white mb-2">{product.nom}</h3>
                        <p className="text-accent-green">${(product.prix || 0).toFixed(2)}</p>
                        {product.description && (
                          <p className="text-white/70 text-sm mt-2 mb-4">{product.description}</p>
                        )}
